...
BASEDIR = Path(__file__).parent  # получаем путь до папки app
UPLOAD_FOLDER = BASEDIR / "static" / "images"  # путь до папки загрузки изображений
...
def add_movie():
    ...
    if form.validate_on_submit():
        ...
        UPLOAD_FOLDER.mkdir(exist_ok=True)  # проверяем наличие папки для загрузки изображений, если такой папки нет - она будет автоматически создана
        image.save(UPLOAD_FOLDER / image_name)  # сохраняем изображение
